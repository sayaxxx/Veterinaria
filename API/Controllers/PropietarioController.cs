...
public class PropietarioController : BaseApiController
{
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
     public async Task<ActionResult<Pager<object>>> propietariosMascotasPag([FromQuery] Params paisParams)
    {
        var entidad = await unitofwork.Propietarios.propietariosMascotas(paisParams.PageIndex, paisParams.PageSize, paisParams.Search);
    ...
    [HttpGet("goldenRetriver")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<object>> goldenRetriver()
    {
        var entidad = await unitofwork.Propietarios.goldenRetriver();
        var dto = mapper.Map<IEnumerable<object>>(entidad);
        return Ok(dto);
    }
    [HttpGet("goldenRetriver")]
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
     public async Task<ActionResult<Pager<object>>> goldenRetriverPag([FromQuery] Params paisParams)
    {
        var entidad = await unitofwork.Propietarios.goldenRetriver(paisParams.PageIndex, paisParams.PageSize, paisParams.Search);
        var listEntidad = mapper.Map<List<object>>(entidad.registros);
        return new Pager<object>(listEntidad, entidad.totalRegistros, paisParams.PageIndex, paisParams.PageSize, paisParams.Search);
    }
    [HttpPost]
